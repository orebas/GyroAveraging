...
GyroAverage-CPU: GyroAverage.cpp ga.h gautils.h
	$(CPP) GyroAverage.cpp   -std=c++14 -Wall  -D_GLIBCXX_USE_CXX11_ABI=0 -O3  -I. -fopenmp -march=native -o GyroAverage-CPU  -I/usr/include/boost169 

GyroAverage-CUDA:  GyroAverage.cpp ga.h gautils.h
	cp -f GyroAverage.cpp GyroAverage.cu 
	nvcc GyroAverage.cu  -std=c++14 -I. -O3  -o GyroAverage-CUDA -D_GLIBCXX_USE_CXX11_ABI=0  -DVIENNACL_WITH_CUDA -lboost_program_options -lOpenCL $(LIBS)-I/usr/include/boost169   -Xcompiler -fopenmp -Xcompiler -I/usr/include/boost169 -DINCL_MATH_CONSTANTS=1  


GyroAverage-Prince-CPU: GyroAverage.cpp ga.h gautils.h
	$(CPP) GyroAverage.cpp  -std=c++14 -Wall  -O3   -fopenmp -march=native -o GyroAverage-CPU  -I${BOOST_INC} -I.    -L${BOOST_LIB}   $(LIBS)


GyroAverage-Prince-OpenCL: GyroAverage.cpp ga.h  gautils.h
	$(CPP) GyroAverage.cpp -std=c++14  -Wall   -O3   -fopenmp -march=native -o GyroAverage-OpenCL -DVIENNACL_WITH_OPENCL -lOpenCL  -I${BOOST_INC}  -I.   -L${BOOST_LIB} $(LIBS)
GyroAverage-Prince-CUDA:  GyroAverage.cpp ga.h gautils.h
	cp -f GyroAverage.cpp GyroAverage.cu
	nvcc GyroAverage.cu    -I. -O3  -o GyroAverage-CUDA   -DVIENNACL_WITH_CUDA  -L{BOOST_LIB}  -lboost_program_options -lOpenCL $(LIBS)   -Xcompiler -fopenmp   -I${CUDA_INC}  -I${BOOST_INC} -I${CUDA_INC}/crt     -L${BOOST_LIB} -DINCL_MATH_CONSTANTS=1
